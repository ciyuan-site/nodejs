...
class Service extends egg.Service {
  ...
  async registerEmail(email) {
    ...
    let data = `AccessKeyId=${ app.config.aliyun.dm.accessKeyId }`
      + `&AccountName=${ encodeURIComponent('passport@ciyuan.site') }&Action=SingleSendMail&AddressType=1&Format=JSON`
      + `&FromAlias=${ encodeURIComponent('次元站') }&ReplyToAddress=false&SignatureMethod=HMAC-SHA1`
      + `&SignatureNonce=${ uuid }&SignatureVersion=1.0&Subject=${ encodeURIComponent('次元站邮箱注册验证码') }`
      + `&TextBody=${ encodeURIComponent('您的邮箱注册验证码为' + code + '，30分钟内有效') }`
